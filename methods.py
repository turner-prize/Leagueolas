...
def updateLOTRWithTablePoints():
    session=CreateSession()
    gw = GetGameweek(session)
    fixtures = session.query(LOTR).filter_by(gameweek=gw).all()
    for f in fixtures:
        scores = session.query(Teams).filter_by(managerId=f.manager1).filter_by(gameweek=gw).filter_by(is_bench=0).all()
        scoreList = []
        for i in scores:
            if i.is_captain==1:
                if TripleCaptain(session,f.manager1,gw):
                    scoreList.append(i.points * 3)
                else:
                    scoreList.append(i.points * 2)
            else:
        ...
        scoresOpponent = session.query(Teams).filter_by(managerId=f.manager2).filter_by(gameweek=gw).filter_by(is_bench=0).all()
        scoreOpponentList = []
        for i in scoresOpponent:
            if i.is_captain==1:
                if TripleCaptain(session,f.manager2,gw):
                    scoreOpponentList.append(i.points * 3)
                else:
                    scoreOpponentList.append(i.points * 2)
            else:
                scoreOpponentList.append(i.points)
        pointsOpponent = sum(scoreOpponentList)
        oph = session.query(LOTR).filter_by(gameweek=gw).filter_by(manager1=f.manager2).first()
        oph = oph.pointhit
        if oph is None:
            oph = 0
